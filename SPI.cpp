...
void SPI::format(int bits, int mode) {
    ...
    if((_ffflags & FormatFlag) == 0){
		...
		// activo flag
		_ffflags |= FormatFlag;
		// si la configuraci�n est� completa, finaliza el registro del driver
...
int SPI::write(int value) {
    ...
    t.length	 = 8;    					// Tama�o en bits
    t.tx_buffer = &value;
    t.rx_buffer = &read;

	//Transmite y espera confirmaci�n de transacci�n completada
    int result = 0;
...
int SPI::write(const char *tx_buffer, int tx_length, char *rx_buffer, int rx_length) {
	...
	spi_transaction_t t;
    memset(&t, 0, sizeof(t));       // Zero out the transaction
	t.length	= max_len * 8;    	// Tama�o en bits
	...
	//Transmite usando DMA! y espera confirmaci�n de transacci�n completada
	if(_nss){
		_nss->write(0);
	}
